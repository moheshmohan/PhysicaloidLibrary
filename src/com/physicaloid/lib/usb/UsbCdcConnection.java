...
public class UsbCdcConnection {
    ...
    public boolean open(UsbVidPid ids, boolean isCdcAcm, int ch) {
        ...
        for(UsbDevice usbdev : mUsbAccess.manager().getDeviceList().values()) {
            if(usbdev.getVendorId() == ids.getVid()) {
            	//mohesh third or condition added
                if(ids.getPid() == 0 || ids.getPid() == usbdev.getProductId() || ids.getPid() == usbdev.getDeviceId()) {
                	//Toast.makeText(mContext,"3rd con pid " + Integer.toHexString(ids.getPid()), Toast.LENGTH_LONG).show();
    ...
    public boolean close() {
    	UsbDeviceConnection con = getConnection();
