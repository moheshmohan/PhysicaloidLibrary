...
public class Physicaloid {
    ...
    public boolean open(UartConfig uart, UsbDevice device) throws RuntimeException {
        synchronized (LOCK) {
        	//Toast.makeText(mContext,"got here with" + device.getDeviceName(), Toast.LENGTH_LONG).show();
            if(mSerial == null) {
                mSerial = new AutoCommunicator().getSerialCommunicator(mContext, device);
                if(mSerial == null) return false;
            }
            //Toast.makeText(mContext,"tryin pid " + Integer.toHexString(device.getDeviceId()), Toast.LENGTH_LONG).show();
            if(mSerial.open(new UsbVidPid(device.getVendorId(), device.getDeviceId()))) {
                mSerial.setUartConfig(uart);
